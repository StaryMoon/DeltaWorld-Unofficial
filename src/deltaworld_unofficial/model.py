"""Compact unofficial PyTorch implementation for A Frame Is Worth One Token: Efficient Generative World Modeling with Delta Tokens.
...
class DeltaTokenPredictor(nn.Module):
    """Task head for generative world modeling with compact temporal delta tokens.
...
class UnofficialModel(nn.Module):
    """PyTorch model interface for A Frame Is Worth One Token: Efficient Generative World Modeling with Delta Tokens."""
    ...
    def __init__(self, config: Optional[StarterConfig] = None) -> None:
        ...
        self.head = DeltaTokenPredictor(self.config)
